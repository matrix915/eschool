...
class Diff {
  public function __construct() {
	...
	var $error = '';
	var $insertedStyle = 'color: red;';
	var $deletedStyle = 'display: none;';
	var $skipDelete = false;

	public function setInsertStyle($styles) {
	  $this->insertedStyle = $styles;
  }

	public function setDeleteStyle($styles) {
	  $this->deletedStyle = $styles;
  }

	public function skipDelete($skip) {
	  $this->skipDelete = $skip;
  }

	public Function SplitString($string, $separators, $end, &$positions) {
		$l = strlen($string);
		$split = array();
		for($p = 0; $p < $l;)
		{
...
{/metadocument}
*/
	public Function Diff($before, $after, &$difference) {
...
*/
  ...
  public Function FormatDiffAsHtml($before, $after, &$difference) {
	  $before = str_replace('<br />', ' ', $before);
	  $after = str_replace('<br />', ' ', $after);
		...
		for($d = 0; $d < $td; ++$d) {
			...
			switch($diff['change']) {
				case '=':
					$html .= '<span class="original-text">'.nl2br(html_entity_decode(substr($before, $diff['position'], $diff['length']))).'</span>';
					break;
				case '-':
				  if(!$this->skipDelete) {
            $html .= '<span class="deleted-text" '.$deletedStyle.'>'.nl2br(html_entity_decode(substr($before, $diff['position'], $diff['length']))).'</span>';
          }
					break;
				case '+':
					$html .= '<span class="added-text"'.$insertedStyle.'>'.nl2br(html_entity_decode(substr($after, $diff['position'], $diff['length']))).'</span>';
					break;
				default:
  ...
  public Function Patch($before, $difference, &$after_patch) {
		$after = '';
		$b = 0;
