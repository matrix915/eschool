...
if (req_get::bool('runStep')) {
    ...
    switch ($step) {
        case 2:
            if (($success = mth_canvas_enrollment::pullWithHomeRoom()) === TRUE) {
                exit('Canvas Grades Updated');
            } elseif (is_null($success)) {
                core_loader::redirect('?runStep=' . $step);
            }
            exit('<span class="error">Problems updating canvas grades!</span>');
        break;
        case 1:
            if (($success = mth_canvas_enrollment::updateHomeRoomZeros()) === TRUE) {
                exit('Canvas Assignment Late & Zeros Updated');
            } elseif (is_null($success)) {
                core_loader::redirect('?runStep=' . $step);
            }
        exit('<span class="error">Problems updating Assignment Late & Zeros!</span>');
        break;
    }
}
