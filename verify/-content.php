...
if(isset($_GET['form']) && isset($_POST['email'])){
    if($user = mth_emailverifier::update(trim($_POST['email']))){
        if($user->sendPasswordResetEmail()){
            core_loader::redirect('/verify/thank');
        }else{
            core_notify::addError('Unable to send account creation email. Please contact admin.');
...
cms_page::setPageContent('<p>Here is one final step to verify your email. Input your verified email address to receive a parent account activation email.<p>');
